...
replace_literal "$old_url" "$web_url" Cargo.toml
...
if $is_github; then
  ...
  if [[ -d .forgejo ]]; then
    ...
    if [[ $reply =~ ^[Yy]$ ]]; then
      ...
      replace_literal 'template/rust' "$secondary_repo_path" .forgejo/renovate.json
  ...
  if [[ -f .forgejo/renovate.json ]]; then
    replace_literal 'template/rust' "$repo_path" .forgejo/renovate.json
